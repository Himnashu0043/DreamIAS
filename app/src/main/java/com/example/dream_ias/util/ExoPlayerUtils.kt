...
class ExoPlayerUtils {
    companion object {
        ...
        fun PlayerView.playVideo(
            ...
            simpleExoPlayer.setMediaItem(mediaItem)
            simpleExoPlayer.prepare()
            simpleExoPlayer.seekTo((simpleExoPlayer.duration - App.app.prefManager.liveStreamTime.toLong()).coerceAtLeast(0))
            simpleExoPlayer.playWhenReady = true
            /*simpleExoPlayer.addListener(object : Player.Listener {
                override fun onPlaybackStateChanged(state: Int) {
                    isBuffering.invoke(state)
                    if (state == Player.STATE_READY) {
                        // Now we can safely seek
                        val liveStreamOffset = App.app.prefManager.liveStreamTime.toLong()
                        val seekPosition = (simpleExoPlayer.duration - liveStreamOffset).coerceAtLeast(0)
           ...
           /* player?.seekTo(
                (player?.duration ?: TIME_UNSET) - App.app.prefManager.liveStreamTime.toLong()
            )*/
            player?.play()
